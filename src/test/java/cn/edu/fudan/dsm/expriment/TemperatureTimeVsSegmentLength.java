 ...
 */
public class TemperatureTimeVsSegmentLength {

    private static Logger logger = Logger.getLogger(TemperatureTimeVsSegmentLength.class);
    ...
    static {
        ...
        stations[18] = new String[]{"57779", "57789", "57874", "57889", "57894", "57896", "57965", "57972", "57974", "57993", "57996", "59065", "59072", "59082", "59087", "59088", "59092", "59097", "59102", "59107", "59116", "59117", "59271", "59278", "59280", "59287", "59293", "59294", "59298", "59303", "59304", "59316", "59317", "59324", "59478", "59485", "59493", "59501"};

        // 20: each from a type
        stations[19] = new String[]{"57348", "57355", "50788", "57696", "53898", "57083", "50353", "52118", "51053", "56459", "58236", "52446", "56691", "53723", "54026", "59058", "53276", "50915", "50618", "57779"};
    }

    public static void main(String args[]) throws IOException {
        TemperatureTimeVsSegmentLength tss = new TemperatureTimeVsSegmentLength();
        double error = 0.05;
        double errorBound = 0.1;
        int duration = 19359;

        tss.searchAllWithPointBasedOpti(19, duration, errorBound, error, 3, true, 50000);
        tss.searchAllWithPointBasedOpti(19, duration, errorBound, error, 5, true, 50000);
        tss.searchAllWithPointBasedOpti(19, duration, errorBound, error, 8, true, 50000);
        tss.searchAllWithPointBasedOpti(19, duration, errorBound, error, 10, true, 50000);
//        tss.searchAllWithPointBasedOpti(19, duration, errorBound, error, 20, true, 50000);
//        tss.searchAllWithPointBasedOpti(19, duration, errorBound, error, 50, true, 50000);
    }

    private void searchAllWithPointBasedOpti(int type, int duration, double errorBound, double error, int segmentLength, boolean z_normalization, int length) throws IOException {
        double[] baseStationData = new double[length];
        int cnt = 0, idx = 0;
        while (cnt < length) {  // concatenate data
            String baseStationFilename = "data/temperature/SURF_CLI_CHN_MUL_DAY-TEM_" + stations[19-idx][0] + "-1960-2012.txt";
            try {
                double[] tmp = readFromFile(baseStationFilename, duration);
                for (int i = 0; i < tmp.length && cnt < length; i++) {
                    baseStationData[cnt++] = tmp[i];
                }
            } catch (FileNotFoundException e) {
                logger.warn(baseStationFilename + " not found! Ignored.");
            }
            if (++idx > 19) break;
        }
...
//        print.println("Base,Target,Pearson,k,b,Final length,Max up bound");
        for (int i = 1; i < stations[type].length; i++) {

            double[] targetStationData = new double[length];
            cnt = 0; idx = 0;
            while (cnt < length) {  // concatenate data
                String targetStationFilename = "data/temperature/SURF_CLI_CHN_MUL_DAY-TEM_" + stations[19-idx][i] + "-1960-2012.txt";
                try {
                    double[] tmp = readFromFile(targetStationFilename, duration);
                    for (int j = 0; j < tmp.length && cnt < length; j++) {
                        targetStationData[cnt++] = tmp[j];
                    }
                } catch (FileNotFoundException e) {
...
//        logger.debug("name = " + filename);
        ...
        for (int i = 19359 - length; i < list.size(); i++) {
            ...
            if (data[i-19359+length] > 500 || data[i-19359+length] < -500) {
                if (i - 19359 + length - 1 >= 0)
                    data[i - 19359 + length] = data[i - 19359 + length - 1];
                else
                    data[i - 19359 + length] = 0;
            }
        }
